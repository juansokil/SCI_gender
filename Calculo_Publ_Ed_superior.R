...
argentina_colab <- dbSendQuery(scopus_ibero, "select year, count(distinct a.ut) from author_address a, article b where a.ut=b.ut and country='Argentina' and address like '%Univ%' group by year")
argentina_colab <-  fetch(argentina_colab, n=-1)
argentina_colab <- cbind(argentina_colab,country)
names(argentina_colab)[names(argentina_colab)=="count(distinct a.ut)"] <- "ed_sup"
...
brazil_colab <- dbSendQuery(scopus_ibero, "select year, count(distinct a.ut) from author_address a, article b where a.ut=b.ut and country='Brazil' and address like '%Univ%' group by year")
brazil_colab <-  fetch(brazil_colab, n=-1)
brazil_colab <- cbind(brazil_colab,country)
names(brazil_colab)[names(brazil_colab)=="count(distinct a.ut)"] <- "ed_sup"
...
cuba_colab <- dbSendQuery(scopus_ibero, "select year, count(distinct a.ut) from author_address a, article b where a.ut=b.ut and country='Cuba' and address like '%Univ%' group by year")
cuba_colab <-  fetch(cuba_colab, n=-1)
cuba_colab <- cbind(cuba_colab,country)
names(cuba_colab)[names(cuba_colab)=="count(distinct a.ut)"] <- "ed_sup"
...
base <- rbind(argentina,brazil, cuba)
